...
public class XxlJobSpringExecutor extends XxlJobExecutor implements ApplicationContextAware, InitializingBean, DisposableBean {
    ...
    private void initJobHandlerMethodRepository(ApplicationContext applicationContext) {
        ...
        for (String beanDefinitionName : beanDefinitionNames) {
            ...
            Map<Method, XxlJob> annotatedMethods = new HashMap<>();   // referred to ：org.springframework.context.event.EventListenerMethodProcessor.processBean
            Method[] methods = bean.getClass().getMethods();
            for (Method method : methods) {
                if (method.isAnnotationPresent(XxlJob.class)){
                    XxlJob xxlJob = method.getAnnotation(XxlJob.class);
                    annotatedMethods.put(method, xxlJob);
                }
            }
